<template>
  <div class="space-y-4 sm:space-y-6">
    ...
    <section class="rounded-2xl border border-white/10 bg-slate-900/70 p-4 sm:rounded-3xl sm:p-6">
      ...
      <div class="mb-5 flex items-center justify-between gap-3">
        <p class="text-sm text-slate-400">
          {{ items.length }} registro<span v-if="items.length !== 1">s</span> carregado<span
            v-if="items.length !== 1">s</span>
        </p>
      ...
      <div v-if="loading"
        class="rounded-2xl border border-white/10 bg-white/5 px-4 py-10 text-center text-sm text-slate-300">
        Carregando dados do backend...
      </div>

      <div v-else-if="!items.length"
        class="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-10 text-center text-sm text-slate-400">
      ...
      <div v-else class="space-y-3">
        <article v-for="item in items" :key="itemKey(item)"
          ...
          <div v-if="isExpanded(item)" class="border-t border-white/10 bg-slate-950/35 px-4 py-5 sm:px-5">
    ...
    <div v-if="isModalOpen" class="mobile-overlay fixed inset-0 z-40 flex items-end justify-center bg-slate-950/75 p-0 sm:items-center sm:p-4 sm:py-6">
      <div
        class="mobile-sheet max-h-[95dvh] w-full max-w-full overflow-y-auto overscroll-contain rounded-t-3xl border border-white/10 bg-slate-900 p-4 shadow-2xl shadow-slate-950/40 sm:max-h-[92dvh] sm:max-w-4xl sm:rounded-3xl sm:p-6">
        <div class="flex items-start justify-between gap-3">
          ...
          <button type="button"
            class="rounded-2xl border border-white/10 p-3 text-slate-300 transition hover:bg-white/5"
            @click="closeModal">
        ...
        <form class="mt-8 grid gap-5 md:grid-cols-2" @submit.prevent="submitForm">
          <div v-for="field in visibleFields" :key="field.name"
            ...
            <div v-if="config.routeName === 'church-schedules' && field.name === 'church_name'" class="space-y-3">
              <input :id="field.name" v-model="formState[field.name]" type="text" :list="churchDatalistId"
              ...
              <datalist :id="churchDatalistId">
                <option v-for="church in partnerChurchOptions" :key="String(church.id ?? church.name)"
            ...
            <select v-else-if="field.type === 'select'" :id="field.name" v-model="formState[field.name]"
            ...
            <textarea v-else-if="field.type === 'textarea'" :id="field.name" v-model="formState[field.name]" rows="4"
              :placeholder="field.placeholder ?? field.label"
              class="block w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" />

            <input v-else :id="field.name" v-model="formState[field.name]" :type="field.type"
              :placeholder="field.placeholder ?? field.label"
              class="block w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" />
          </div>
          ...
          <div class="md:col-span-2 flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
            <button type="button"
              class="rounded-2xl border border-white/10 px-4 py-3 text-sm font-medium text-slate-300 transition hover:bg-white/5"
              @click="closeModal">
              Cancelar
            </button>
            <button type="submit" :disabled="submitting"
              class="rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-70">
...
import { ApiError, buildUrl, createResource, deleteResource, getResourceCollection, listResources, updateResource, type ResourceRecord } from '@/api'
...
function getFieldOptions(field: ResourceConfig['fields'][number]) {
  ...
  if (props.config.routeName === 'gideon-friends' && field.name === 'contacted_by_id') {
    return memberOptions.value.map((member) => ({
      label: `${String(member.full_name ?? 'Membro sem nome')} • ${String(member.classification ?? '')}`,
      value: String(member.id ?? ''),
    }))
  }

  if (['attendances', 'minutes'].includes(props.config.routeName) && field.name === 'meeting_id') {
...
async function loadPartnerChurchOptions() {
  ...
  async function loadAttendanceDependencies() {
    if (!['attendances', 'minutes', 'gideon-friends'].includes(props.config.routeName) || !token.value) {
      return
    }

    try {
      const shouldLoadMeetings = ['attendances', 'minutes'].includes(props.config.routeName)
      const shouldLoadMembers = ['attendances', 'gideon-friends'].includes(props.config.routeName)

      const meetingsPromise = shouldLoadMeetings
        ? getResourceCollection<ResourceRecord>('/api/meetings/', token.value)
        : Promise.resolve({ items: [], count: 0, next: null, previous: null })

      const membersPromise = shouldLoadMembers
        ? getResourceCollection<ResourceRecord>('/api/members/', token.value)
        : Promise.resolve({ items: [], count: 0, next: null, previous: null })

      const [membersResponse, meetingsResponse] = await Promise.all([membersPromise, meetingsPromise])
...
async function fetchProtectedFile(path: string) {
  if (!token.value) {
    errorMessage.value = 'Sessão expirada. Faça login novamente.'
    return null
  }
...
async function openMinutePdf(item: ResourceRecord) {
  const id = item.id as number | string | undefined

  if (id == null) {
    errorMessage.value = 'Não foi possível identificar esta ata para gerar PDF.'
    return
  }

  try {
    const response = await fetchProtectedFile(`/api/minutes/${id}/pdf/`)
    if (!response) return
...
async function openMinutePrint(item: ResourceRecord) {
  const id = item.id as number | string | undefined

  if (id == null) {
    errorMessage.value = 'Não foi possível identificar esta ata para impressão.'
    return
  }

  try {
    const response = await fetchProtectedFile(`/api/minutes/${id}/print/`)
    if (!response) return
    ...
    errorMessage.value = 'Não foi possível abrir a visualização de impressão desta ata.'
  }
}
