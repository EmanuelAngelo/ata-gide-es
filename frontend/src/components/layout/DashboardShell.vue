<template>
  <div class="dashboard-shell flex h-dvh max-h-dvh overflow-hidden bg-slate-950 text-slate-100">
    <aside
      :class="[
        'fixed inset-y-0 left-0 z-30 flex h-dvh w-72 max-w-[calc(100%-1rem)] shrink-0 flex-col border-r border-white/10 bg-slate-900/95 px-3 py-5 backdrop-blur transition-transform duration-300 sm:max-w-xs sm:px-4 sm:py-6 lg:static lg:max-w-none lg:translate-x-0',
        sidebarOpen ? 'translate-x-0' : '-translate-x-full',
      ]"
    >
      <div class="flex shrink-0 items-center justify-between gap-2 px-1 sm:px-2">
        <div class="min-w-0">
      ...
      <div class="mt-8 shrink-0 rounded-2xl border border-white/10 bg-white/5 p-4">
        <p class="text-sm font-medium text-white">{{ username || 'Usuário autenticado' }}</p>
      </div>

      <nav class="mt-8 min-h-0 space-y-2 overflow-y-auto overscroll-contain">
        <button
          v-for="item in items"
          :key="item.key"
          type="button"
        ...
        <button
          type="button"
          class="flex w-full items-center gap-3 rounded-2xl border border-white/10 px-4 py-3 text-left text-sm font-medium text-slate-200 transition hover:bg-white/5"
          @click="$emit('logout')"
        >
          <span class="mdi mdi-logout shrink-0 text-xl" />
    ...
    <div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      <header class="shrink-0 border-b border-white/10 bg-slate-950/80 px-3 py-3 backdrop-blur sm:px-6 sm:py-4 lg:px-10">
      ...
      <main class="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-3 py-4 sm:px-6 sm:py-6 lg:px-10 lg:py-8">
        <slot />
      </main>
    </div>
